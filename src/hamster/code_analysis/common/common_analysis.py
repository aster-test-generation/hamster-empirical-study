...
class CommonAnalysis:
    ...
    def __get_inheritance_chain(self, qualified_class_name: str, filter_by_modifier=None) -> List[str]:
        """
        ...
        if filter_by_modifier is None:
            filter_by_modifier = []
        inheritance_chain = []
        extends_list = (self.analysis.
                        get_extended_classes(qualified_class_name=qualified_class_name))
        for extend in extends_list:
            ...
            if extend_class_details:
                if len(filter_by_modifier) > 0:
                    for modifier in extend_class_details.modifiers:
                        if modifier in filter_by_modifier:
                            inheritance_chain.append(extend)
                            inheritance_chain.extend(self.__get_inheritance_chain(qualified_class_name=extend))
                else:
                    inheritance_chain.append(extend)
                    inheritance_chain.extend(self.__get_inheritance_chain(qualified_class_name=extend))
        return inheritance_chain

    def get_method_in_class_including_parents_and_implemented_classes(self, qualified_class_name: str,
                                                                      method_signature: str) -> Optional[JCallable]:
        """
        Get JCallable object including inherited and implemented methods
        Args:
        ...
        if method_details is not None:
            ...
            if class_details is not None:
                # If a concrete class
                if 'abstract' not in class_details.modifiers and not class_details.is_interface:
                    # Check all the inherited methods
                    inherited_classes = self.__get_inheritance_chain(qualified_class_name)
                    for inherited_class in inherited_classes:
                        inherited_method_details = self.analysis.get_method(inherited_class, method_signature)
                        if inherited_method_details is not None:
                            return inherited_method_details
                else:
                    # Check all implemented classes in case of a non-concrete class
                    concrete_classes = self.get_concrete_classes(qualified_class_name)
                    for concrete_class in concrete_classes:
                        concrete_method_details = self.analysis.get_method(concrete_class, method_signature)
                        if concrete_method_details is not None:
                            return concrete_method_details

        return None

    def get_concrete_classes(self, qualified_class_name: str) -> List[str]:
        concrete_classes = []
        all_sub_classes = (self.analysis.
                           get_sub_classes(qualified_class_name=qualified_class_name))
        for sub_class in all_sub_classes:
            if not all_sub_classes[sub_class].is_interface and \
                    'abstract' not in all_sub_classes[sub_class] and \
                    sub_class != qualified_class_name:
