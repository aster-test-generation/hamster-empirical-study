...
class FocalClassMethod:
    ...
    def identify_focal_class_and_ui_api_test(self, test_class_name: str, test_method_signature: str,
                                             setup_methods: Dict[str, List[str]]) -> Tuple[
        ...
        if setup_methods:
            for declaring_class, method_signatures in setup_methods.items():
                ...
                for setup_method_signature in method_signatures:
                    if setup_method_signature is None:
                        continue
        ...
        for f_class in focal_classes:
            if focal_classes[f_class] in all_focal_methods:
                ...
                if not is_present:
                    final_focal_methods = []
                    for f in all_focal_methods[focal_classes[f_class]]:
                        if (self.common_analysis.
                                get_method_in_class_including_parents_and_implemented_classes(focal_classes[f_class],
                                                                                              f) is not None):
                            final_focal_methods.append(f)
                    if len(final_focal_methods) > 0:
                        focal_class_model_object.append(FocalClass(focal_class=focal_classes[f_class],
                                                                   focal_method_names=final_focal_methods))
            else:
    ...
    def __get_focal_class_and_method(self, class_name: str, method_signature: str,
        ...
        helper_methods = Reachability(self.analysis).get_helper_methods(
        ...
        def get_frameworks_for(class_to_check: str) -> List[TestingFramework]:
            ...
            try:
                frameworks = self.common_analysis.get_testing_frameworks_for_class(class_to_check)
            except Exception:
                frameworks = []
