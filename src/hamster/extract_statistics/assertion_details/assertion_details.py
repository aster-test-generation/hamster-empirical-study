...
from hamster.code_analysis.model.models import ProjectAnalysis, AssertionType, CallAndAssertionSequenceDetails, AppType, \
    CallableDetails, AssertionDetails as ModelAssertionDetails
...
class AssertionDetails:
    ...
    def extract_details(self) -> dict | None | str:
        ...
        assertion_counts_by_type_per_app_type: Dict[AppType, Dict[AssertionType, int]] = defaultdict(
            Counter)  # Per application type
        ...
        print("Processing assertion statistics...")
            ...
            for project_analysis in p.track(self.all_project_analyses, total=len(self.all_project_analyses)):
                ...
                for test_class in project_analysis.test_class_analyses:
                    for test_method in test_class.test_method_analyses:
                        ...
                        top_test_blocks.add(num_test_blocks, test_method.method_signature,
                                            test_class.qualified_class_name, project_analysis.dataset_name)
                        ...
                        for seq in seqs:
                            ...
                            top_callable_seq_lengths.add(len(call_details), test_method.method_signature,
                                                         test_class.qualified_class_name, project_analysis.dataset_name)
                            ...
                            filtered_assert_details = [ad for ad in assert_details if
                                                       set(ad.assertion_type) - ignored_types]
                            ...
                            if filtered_assert_details:
                                ...
                                for k in range(len(filtered_assert_details) - 1):
                                    if (
                                            ...
                                            and AssertionType.WRAPPER not in filtered_assert_details[
                                        k + 1].assertion_type
                                    ):
                                ...
                                if curr_fluid_chain_length > 0:
                                    if cd.secondary_assertion:
                                        ...
                                        if curr_fluid_chain_length > 2:
                                            ...
                                            top_fluid_chain_lengths.add(curr_fluid_chain_length,
                                                                        test_method.method_signature,
                                                                        test_class.qualified_class_name,
                                                                        project_analysis.dataset_name)
                        ...
                        if curr_fluid_chain_length > 2:
                            fluid_chain_lengths.append(curr_fluid_chain_length)
                            top_fluid_chain_lengths.add(curr_fluid_chain_length, test_method.method_signature,
                                                        test_class.qualified_class_name, project_analysis.dataset_name)
                            has_fluid_chain = True
                        ...
                        percentage_ncloc = (
                                    num_assertions / test_method.ncloc_with_helpers * 100) if test_method.ncloc_with_helpers > 0 else 0
        ...
        number_assertion_sequences_per_method_stats = ExtractStatisticsUtils.get_summary_stats(
            number_assertion_sequences_per_method)
        ...
        secondary_assertions_per_method_stats = ExtractStatisticsUtils.get_summary_stats(
            secondary_assertions_per_method)
        secondary_getter_assertions_per_method_stats = ExtractStatisticsUtils.get_summary_stats(
            secondary_getter_assertions_per_method)
        secondary_assertions_per_method_with_fluid_stats = ExtractStatisticsUtils.get_summary_stats(
            secondary_assertions_per_method_with_fluid)
        assertions_in_helper_per_method_stats = ExtractStatisticsUtils.get_summary_stats(
            assertions_in_helper_per_method)
        assertions_in_helper_per_method_with_helper_stats = ExtractStatisticsUtils.get_summary_stats(
            assertions_in_helper_per_method_with_helper)
        assertion_percentage_entities_per_method_stats = ExtractStatisticsUtils.get_summary_stats(
            assertion_percentage_entities_per_method)
        assertion_percentage_ncloc_per_method_stats = ExtractStatisticsUtils.get_summary_stats(
            assertion_percentage_ncloc_per_method)
        ...
        if self.output_format == OutputFormatType.JSON_PDF_FIGURES:
            ...
            utils.get_box_plot(number_test_blocks_per_method, ["# of test blocks per method"],
                               "Box plot of test blocks per test method", "test_blocks_per_method_plot")
            utils.get_box_plot(number_assertions_per_method, ["# of assertions per method"],
                               "Box plot of assertions per test method", "assertions_per_method_box_plot")
            utils.get_box_plot(number_assertion_sequences_per_method, ["# of assertion sequences per method"],
                               "Box plot of assertion sequences per method", "assertion_sequences_per_method_box_plot")
            utils.get_box_plot(length_callable_sequences, ["Callable sequence lengths"],
                               "Box plot of callable sequence lengths", "callable_sequence_length_box_plot")
            utils.get_box_plot(length_assertion_sequences, ["Assertion sequence lengths"],
                               "Box plot of assertion sequence lengths", "assertion_sequence_length_box_plot")
            utils.get_box_plot(secondary_assertions_per_method, ["# of secondary assertions per method"],
                               "Box plot of secondary assertions per method",
                               "secondary_assertions_per_method_box_plot")
            utils.get_box_plot(secondary_getter_assertions_per_method, ["# of secondary getter assertions per method"],
                               "Box plot of secondary getter assertions per method",
                               "secondary_getter_assertions_per_method_box_plot")
            utils.get_box_plot(assertions_in_helper_per_method, ["# of assertions in helper per method"],
                               "Box plot of assertions in helper per method",
                               "assertions_in_helper_per_method_box_plot")
            utils.get_box_plot(assertion_percentage_entities_per_method, ["Assertion % entities per method"],
                               "Box plot of assertion percentage entities per test method",
                               "assertion_percentage_entities_per_method_box_plot")
            utils.get_box_plot(assertion_percentage_ncloc_per_method, ["Assertion % NCLOC per method"],
                               "Box plot of assertion percentage NCLOC per test method",
                               "assertion_percentage_ncloc_per_method_box_plot")
            if len(fluid_chain_lengths) > 0:
                utils.get_box_plot(fluid_chain_lengths, ["Fluid chain lengths"],
                                   "Box plot of fluid assertion chain lengths", "fluid_chain_lengths_box_plot")
