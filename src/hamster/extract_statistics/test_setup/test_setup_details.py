...
class TestSetupDetails:
    ...
    def extract_details(self) -> dict | str:
        ...
        total_projects = 0
        total_mocking = 0
        total_setup_methods = 0
        inherited_setup_methods = 0
                ...
                for test_class in project_analysis.test_class_analyses:
                    ...
                    for setup_analysis in test_class.setup_analyses:
                        if setup_analysis is None:
                            continue
                        total_setup_methods += 1
                        if setup_analysis.qualified_class_name != test_class.qualified_class_name:
                            inherited_setup_methods += 1
                    ...
                    setup_per_class.append(setup_count)
        before_class_setup_percentage = before_class_setup_percentage / len(setup_execution_order) if len(
            setup_execution_order) > 0 else 0
        before_method_setup_percentage = before_method_setup_percentage / len(setup_execution_order) if len(
            setup_execution_order) > 0 else 0

        inherited_setup_percentage = (inherited_setup_methods / total_setup_methods
                                      if total_setup_methods > 0 else 0)
        ...
        return {"ncloc": self.extract_statistics_util.get_summary_stats(ncloc_per_method),
                "total_setup_methods": total_setup_methods,
                "total_setup_method_with_mocking": total_mocking,
                "avg_setup_methods": total_setup_methods / total_projects if total_projects > 0 else 0,
                "inherited_setup_methods": inherited_setup_methods,
                "inherited_setup_percentage": inherited_setup_percentage,
                ...
                "mocks_created": self.extract_statistics_util.get_summary_stats(mocks_created) if len(
                    mocks_created) > 0 else None,
                ...
                "setup_per_class": self.extract_statistics_util.get_summary_stats(setup_per_class)}
