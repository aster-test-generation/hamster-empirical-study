...
class TestTeardownDetails:
    ...
    def extract_details(self) -> dict | str:
        ...
        code_details = []
        total_projects = 0
        total_teardown_methods = 0
        inherited_teardown_methods = 0
                ...
                for test_class in project_analysis.test_class_analyses:
                    ...
                    for teardown_analysis in test_class.teardown_analyses:
                        if teardown_analysis is None:
                            continue
                        total_teardown_methods += 1
                        if teardown_analysis.qualified_class_name != test_class.qualified_class_name:
                            inherited_teardown_methods += 1
                        size_bucket = None
                        teardown_count += 1
                    ...
                    teardown_per_class.append(teardown_count)
        ...
        inherited_teardown_percentage = (inherited_teardown_methods / total_teardown_methods
                                         if total_teardown_methods > 0 else 0)
        ...
        return {"ncloc": self.extract_statistics_util.get_summary_stats(ncloc_per_method),
                "total_teardown_methods": total_teardown_methods,
                "avg_teardown_methods": total_teardown_methods / total_projects if total_projects > 0 else 0,
                "inherited_teardown_methods": inherited_teardown_methods,
                "inherited_teardown_percentage": inherited_teardown_percentage,
                "library_call_details": self.extract_statistics_util.get_summary_stats(library_called),
                "constructor_calls": self.extract_statistics_util.get_summary_stats(constructor_called),
                "cleanup_calls": self.extract_statistics_util.get_summary_stats(cleanup),
                "cleanup_type_distribution": self.extract_statistics_util.get_distribution_percentage(
                    cleanup_type_details) if len(cleanup_type_details) > 0 else None,
                "cyclomatic_complexity": self.extract_statistics_util.get_summary_stats(cyclomatic_complexity),
                "objects_created": self.extract_statistics_util.get_summary_stats(objects_created),
                # "mocks_created": self.extract_statistics_util.get_summary_stats(mocks_created) if len(mocks_created) > 0 else None,
                ...
                "teardown_per_class": self.extract_statistics_util.get_summary_stats(teardown_per_class)}
