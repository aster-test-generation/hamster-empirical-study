...
from hamster.code_analysis.model.models import ProjectAnalysis, InputType, AppType, TestInput, TestClassAnalysis, \
    TestMethodAnalysis, SetupAnalysis
...
class TestInputDetails:
    ...
    def extract_details(self) -> dict | None | str:
            ...
            for project_analysis in p.track(self.all_project_analyses, total=len(self.all_project_analyses)):
                ...
                for test_class in project_analysis.test_class_analyses:
                    ...
                    for test_method in test_class.test_method_analyses:
                            ...
                            if non_ignored:
                                ...
                                if len(non_ignored) == 1:
                                    ...
                                    ambiguous_input_counts[
                                        ", ".join([ti.receiver_type, ti.method_signature, ti.method_name])] += 1
                        ...
                        top_test_inputs_methods.add(combined_num, test_method.method_signature,
                        ...
                        num_assertions = sum(
                            len(seq.assertion_details or []) for seq in test_method.call_assertion_sequences or [])
        ...
        num_constructor_calls_per_method_stats = ExtractStatisticsUtils.get_summary_stats(
            num_constructor_calls_per_method)
        num_application_calls_per_method_stats = ExtractStatisticsUtils.get_summary_stats(
            num_application_calls_per_method)
        ...
        if self.output_format == OutputFormatType.JSON_PDF_FIGURES:
            ...
            utils.get_box_plot(number_test_inputs_per_method, ["# of test inputs per method"],
                               "Box plot of test inputs per test method", "test_inputs_per_method_plot")
            utils.get_box_plot(ncloc_per_method, ["NCLOC per method"], "Box plot of NCLOC per test method",
                               "ncloc_per_method_plot")
            utils.get_box_plot(num_constructor_calls_per_method, ["# of constructor calls per method"],
                               "Box plot of constructor calls per test method", "constructor_calls_per_method_plot")
            utils.get_box_plot(num_application_calls_per_method, ["# of application calls per method"],
                               "Box plot of application calls per test method", "application_calls_per_method_plot")
            utils.get_box_plot(num_focal_methods_per_method, ["# of focal methods per method"],
                               "Box plot of focal methods per test method", "focal_methods_per_method_plot")
            utils.get_box_plot(num_mocks_created_per_method, ["# of mocked resources per method"],
                               "Box plot of mocked resources per test method", "mocked_resources_per_method_plot")
